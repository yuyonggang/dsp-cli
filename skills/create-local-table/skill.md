...
```
/create-local-table --name CUSTOMER_TABLE --space YOUR_SPACE_ID
```
Creates a table with default columns: ID, NAME, DESCRIPTION, AMOUNT, CREATED_DATE

### Example 2: Custom columns
```
/create-local-table --name PRODUCT_TABLE --space YOUR_SPACE_ID --columns "PRODUCT_ID:cds.String:10:key:required,PRODUCT_NAME:cds.String:100:required,PRICE:cds.Decimal:15:2,STOCK:cds.Integer"
```
...
```
/create-local-table --name ORDER_HEADER --space YOUR_SPACE_ID --label "Order Header Table"
```
